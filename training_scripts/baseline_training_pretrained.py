...
class Preprocessor():
    ...
    def modify_state(self, obs, info):
        ...
        obs = np.hstack((robot_qpos, 
                         ...
                         task_onehot))
  # --- NEW PADDING LOGIC ---
        target_dim = 89  # The magic number your model wants
...
def main():
    ...
    model = DDPG_Gaussian(
        input_dim= 89,  # 87 Observation dimension (no task onehot from preprocessor)
        action_dim=12,
    ...
    pretrained_path = Path(__file__).parent.parent / "exp_local" / "booster" / "checkpoints" / "run4" / "checkpoint_100000.pkl"
    ...
    train_with_checkpoints(
        ...
        timesteps=600000,
        checkpoint_dir=Path(__file__).parent.parent / "exp_local" / "booster" / "checkpoints" / "run6",
    ...
    exp_dir = Path(__file__).parent.parent / "exp_local" / "booster" / "checkpoints" / "run6"
