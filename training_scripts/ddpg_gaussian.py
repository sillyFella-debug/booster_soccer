...
class DDPG_Gaussian(nn.Module):
    ...
    def load_pretrained_actor(self, checkpoint_path):
        """Load actor weights from converted_model.pt (TorchScript format)."""
        try:
            ...
            # Map hidden layers and layer norms
            for i in range(len(self.actor.hidden_layers)):
                ...
                # If first layer has more input dims than expected, truncate
                # if i == 0 and w.shape[1] != self.actor.hidden_layers[i].in_features:
                #     print(f"  Truncating first layer weights from {w.shape[1]} to {self.actor.hidden_layers[i].in_features} input dims")
                #     w = w[:, :self.actor.hidden_layers[i].in_features]
            ...
            print(f"✓ Successfully loaded pre-trained actor from {checkpoint_path}")
            return True
        except Exception as e:
            print(f"Error loading pretrained actor: {e}")
            import traceback
            traceback.print_exc()
            return False
